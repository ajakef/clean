...
stream = clean.make_synth_stream_local(src_xyz = src_xyz, amp = src_amp, fl = fl, fh = fh, Nt = 3000)
...
freqs = np.arange(5)
crossSpec, FT, freqs, dfN, dfD = clean.calc_cross_spectrum(stream, win_length_sec = 4, freq_bin_width = 1)
steeringVectors = clean.make_steering_vectors_local(stream, src_xyz, freqs = freqs)
...
stream = clean.make_synth_stream_local(src_xyz = src_xyz, amp = src_amp, fl = fl, fh = fh, Nt = 6000, uncorrelatedNoiseAmp=.30,
                                       #Nx = 8, dx = 0.01, Ny = 1, dy = 0.03)
                                       x = sta_x, y = sta_y, c = 0.34)
freq_max = 40
crossSpec, FT, freqList, dfN, dfD = clean.calc_cross_spectrum(stream, win_length_sec = 2, freq_bin_width = 1)
        ## drop freqs outside the user-defined range
w = np.where((freqList < freq_max) & (freqList > 0))[0]
crossSpec = crossSpec[:,:,w]
freqList = freqList[w]
steeringVectors = clean.make_steering_vectors_local(stream, src_xyz, freqs = freqList, c = 0.34)
station_distances = clean.calc_station_pair_distance(stream)

#%% run the clean analysis
result = clean.clean(stream, verbose = True, phi=0.1, win_length_sec=2, freq_bin_width=1,
...
stream_geometry = clean.get_geometry(stream) ## check to make sure the coordinates are correct
